        ...
        public void accept(BranchNode<?, LogEventNodeType> logEvent)
        {
            ...
            try
            {
                ...
                try
                {
                    ...
                    logEvent.getUnmodifiableNodeList(LogEventNodeType.properties).forEach(ExceptionCatchedConsumer.wrap(p -> {
                        session.persist(p);
                        session.persist(p.get(LogPropertyNodeType.correlatedLogEvent));
                    }));
